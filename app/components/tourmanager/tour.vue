...
const confirmRename = (newName?: string) => {
  if (newName) props.tour.name = newName;
...
defineEmits(['deleteTour', 'addBuy', 'addSell', 'openSellModal', 'openCostModal']);
</script>

<template>
  <div class="flex justify-between items-center">
    <div class="flex flex-row gap-2 items-center">
      <h2 v-if="!renameOpen" @click="toggleRename()" class="text-lg font-bold hover:cursor-pointer">{{ tour.name }}</h2>
      <input v-if="renameOpen" @keyup.enter="event => confirmRename((event?.target as HTMLInputElement).value)"
        name="location" placeholder="Tour name" class="border p-1" />
  ...
  <SellModal v-if="sellModalOpen" :commodity="sellModal.commodity" :amount="sellModal.amount"
    @confirm-sell="confirmSell" @cancel-sell="cancelSell" />
</template>
